...
- Branch or worktree identity binds each claimed `NEXT` item to its development branch so parallel changes remain locatable. In `REMOTE` tracking mode each machine keeps a local projection of this file refreshed from the tracker and Gate records; the tracker wins any disagreement, and a Git-level conflict on this file is resolved by regenerating the projection from authoritative sources, never by hand-picking sides. A divergent worktree's Stage copy is not live state until the canonical writer reconciles it.
