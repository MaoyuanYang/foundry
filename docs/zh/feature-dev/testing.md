...
此分支期间，`STAGE.md` 使用 `TEST_DESIGN`；只有权威 Test Gate 记录存在后，才添加其链接/revision 投影，场景与证据仍留在 Test Design 和 Gate 记录中。
...
Happy Path、Alternative Flows、边界、错误、非法输入、认证、授权、幂等、并发、事务、一致性、重试/超时、重复请求/消息、失败恢复、迁移、向后兼容、回归、并行工作项集成（合并顺序交互、共享契约回归、合并后联合回归）——有 UI 时再加：用户交互、表单校验、加载、错误/空状态、可访问性、导航、E2E、视觉回归。不适用项记简短理由，而非机械造场景。
...
11. 当其他 `NEXT` 工作项被并行认领时，其与本项的集成交互已有场景覆盖或记 `N/A - no concurrent work items`。
