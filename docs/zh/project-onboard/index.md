...
现有仓库
    ↓
1. 检查仓库          结构、语言、入口、配置、测试、CI、版本控制历史,
                     托管在 GitHub 时还包括其协作面
    ↓
...
4. 对比代码与文档    用 Observed / Inferred / Unknown 标注发现
    ↓
5. 询问用户          只问仓库无法回答的问题;从零创建的文档确认语言;
                     在 GitHub 上,询问是否采纳协作
    ↓
...
```text
运行时行为 > 测试 > 代码 > 迁移/配置 > issue 与 PR 历史 > 文档 > 推断
```

issue 与 pull request 历史 —— 当托管平台提供时 —— 是同时代的人类记录:一个已关闭
PR 的讨论常常解释代码本身无法解释的意图。当文档与现实冲突时,现实获胜,并记录
冲突。为了让基线诚实而不引入官僚机制,发现只带三个标签之一:
...
在托管于 GitHub 且 `gh` CLI 可用的仓库上,访谈会问一次是否采纳 GitHub 协作;决定
记录在恢复出的 Roadmap 的 `## Tracking` 中。启用后,恢复与托管平台对账:已合并的
pull request 历史佐证 `Done`;打开的 issue 被收编为候选 `Draft` 条目(链接而非
重复);已有的 Projects 看板和 milestone 被采为恢复后 Roadmap 的镜像。

## 产出物

从零创建的文档以与用户确认的语言书写(默认英文);修复的文档沿用其原有语言。
...
- 未经用户明确授权,不做破坏性或远程操作。协作决定授权采纳托管平台的镜像;
  其余远程操作仍需逐一询问。
