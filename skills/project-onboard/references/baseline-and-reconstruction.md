...
- All three language dimensions (Documentation, Engineering, and Product Content) across every applicable `AGENTS.md` rule, per the canonical contract in this Skill's `SKILL.md` Language Policy: record evidence-supported values or allowed Product Content states together with scope, evidence, named authority, adoption state, and synchronization status.
