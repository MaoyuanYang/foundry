...
8. Apply the resolved Language Policy: Documentation Language controls formal artifact prose, Engineering Language controls technical names and engineering surfaces, and Product Content Language controls user-facing copy and localized values. Product Content records actual BCP-47 value(s), `UNKNOWN - <resolution action>` for a potentially relevant unresolved surface, or `N/A - no product-content surface` when the scoped survey confirms absence. Never mass-translate existing comments, docs, identifiers, or product content.

Before updating a formal artifact, inspect its existing formal-prose language. Excluding clearly labeled exact Product Content, a mixed document, a difference from the resolved Documentation Language, or an edit that would add a second prose language or require translation is `CONFLICT` and `STOP` in either direction. Resume only after named-authority approval of one whole-document language and separate authorization for the required translation/update scope. In a mixed repository, new surfaces use the English defaults while user-facing product content keeps its established language. Preserve a non-English Documentation or Engineering override only when it records a valid scoped BCP-47 value, named authority, explicit `ADOPTED` evidence, source, and date with no conflicting policy; otherwise it is blocking `CONFLICT` and `STOP` regardless of nested precedence. After named-authority resolution, record actual Documentation and Engineering BCP-47 values and the Product Content value/state, then synchronize the authoritative AGENTS policy location, Baseline, and AS-IS Spec metadata. Conversation language never overrides policy.
