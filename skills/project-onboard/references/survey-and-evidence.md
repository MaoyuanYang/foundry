...
Record detected BCP-47 values, consistency, scope, evidence, the named authority, and adoption state. Product Content Language has no default: use actual evidence-supported BCP-47 value(s), `UNKNOWN - <resolution action>` for a potentially relevant unresolved surface, or `N/A - no product-content surface` only after the complete scoped survey confirms absence. Conversation or prompt language never overrides repository policy. Preserve existing identifiers and Product Content Language; never mass-translate comments, docs, identifiers, or product content. In mixed repositories, new surfaces use `documentation_language = en` and `engineering_language = en` unless a named `Maintainer Decision Authority` explicitly approves and `ADOPTS` a scoped override. Every new ID, slug, or technical name uses English and fits repository shape unless that explicit scoped Engineering Language override applies.

A non-English Documentation or Engineering rule is resolved only when it records a valid scoped BCP-47 value, named authority, explicit `ADOPTED` evidence, source, and date with no conflicting policy; preserve that override, otherwise record `CONFLICT` and `STOP` regardless of nested precedence. Before updating a formal artifact, inspect its existing formal-prose language. Excluding clearly labeled exact Product Content, a mixed document, a difference from the resolved Documentation Language, or an edit that would add a second prose language or require translation is `CONFLICT` and `STOP` in either direction. Resume only after named-authority approval of one whole-document language and separate authorization for the required translation/update scope. After resolution, synchronize actual Documentation and Engineering BCP-47 values and the Product Content value/state, scope, authority, and date in the authoritative `AGENTS.md` policy location, the Baseline, and each AS-IS Spec. Complete onboarding requires explicit Language Policy adoption by the named `Maintainer Decision Authority`; applicable Product Content `UNKNOWN` remains incomplete, while evidenced `N/A - no product-content surface` is resolved. Under the defaults, render the `## Language Policy` section in English; after an approved Documentation Language override, render its prose consistently in that language while preserving the exact ASCII policy keys.
