...
Record `DONE Status: PASS` and `Roadmap Status: DONE` together only when every referenced `DONE` condition is met; otherwise MUST NOT record `DONE`. Delivery state progresses `REVIEW -> READY FOR PR | READY FOR DELIVERY -> DELIVERED`; `DELIVERED` means the confirmed Definition of Done is met (the authorized PR is opened/approved/merged, or the explicitly adopted no-PR delivery record exists), and it is recorded together with `DONE Status: PASS` and `Roadmap Status: DONE`. On success, report Gate revisions and verification/delivery evidence, then `STOP`.
