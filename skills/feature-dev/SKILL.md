...
After scope and local-write authorization are valid, create or incrementally adopt root `STAGE.md` from the [Project Stage template](assets/stage.template.md). Use these exact Feature-stage tokens: `PREFLIGHT`, `WORK_ITEM_BINDING`, `SPEC_REFINEMENT`, `UI_REFINEMENT`, `TEST_DESIGN`, `IMPLEMENTATION_PLAN`, `CODING_TESTING`, `REVIEW`, `DOCUMENTATION_SYNC`, `DELIVERY`, and `COMPLETE`. Increment the snapshot only on assignment, a meaningful transition, block/resume, handoff, or completion. Serialize every Stage write through the repository lock or designated canonical writer; otherwise compare the revision and SHA-256 read immediately before writing and abort/reconcile on change. Allocate `A-xxx` under the same guard. A branch-local or divergent-worktree copy is not live Stage state until the canonical writer reconciles it. Before every update, reread Stage and the Work Status authority; change only the current activity and directly affected coordination rows. Preserve unrelated members. An unexplained second claim on the same work item is `CONFLICT` unless explicit collaboration and responsibility boundaries are recorded.
